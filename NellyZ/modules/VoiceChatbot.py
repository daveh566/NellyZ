...
from NellyZ.services.pyrogram import pbot as LYCIA
...
@LYCIA.on_message(filters.command("nelly"))
async def Lycia(_, message):
    ...
    m = await message.reply_text("Nellyz Is Best...")
    try:
        L = await fetch(
            f"https://api.affiliateplus.xyz/api/chatbot?message={lycia}&botname=Nelly&ownername=Aspirer&user=1"
        )
        chatbot = L["message"]
